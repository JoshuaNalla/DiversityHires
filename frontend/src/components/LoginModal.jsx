...
const LoginModal = ({ onClose, onSwitchToSignup }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md" style={{ fontFamily: "'Inter', sans-serif" }}>
            <main className="w-full max-w-md bg-[#3D3C5C] rounded-2xl shadow-2xl overflow-hidden p-8 md:p-10 border border-[#5A587A] relative animate-in fade-in zoom-in duration-200">
                {/* Close Button */}
                <button 
                    onClick={onClose}
                ...
                <div className="text-center mb-8 mt-2">
                    <h2 className="text-2xl font-semibold text-[#F5EAEB]">Log in to Interview Prep</h2>
                </div>
                ...
                <div className="mt-8 text-center text-sm">
                    ...
                    <button onClick={onSwitchToSignup} className="text-[#C2A3A6] hover:underline font-medium opacity-90 hover:opacity-100 transition-opacity">Sign up here</button>
                </div>
            </main>
        </div>
    );
