...
export default function FaceEmotionReader({ onReady }) {
  ...
  const stopMedia = () => {
    // Utilize the detached streamRef to ensure hardware unhooks correctly when unmounted
  ...
  const captureAndAnalyze = useCallback(async () => {
    ...
    canvas.toBlob(async (blob) => {
      ...
      try {
        const response = await fetch('http://localhost:8000/api/interview/analyze-frame', {
          method: 'POST',
          body: formData
        });
  ...
  return (
    <div className="dashboard-container h-full w-full max-h-screen">
      <header className="dashboard-header pl-6 lg:pl-10">
      ...
      <main className="flex flex-col gap-6 pl-6 lg:pl-10 mt-2 pb-4 h-full overflow-hidden">


        <section className="panel video-panel !flex-1 min-h-[40%] flex flex-col">
          <h3 className="text-lg font-semibold text-slate-100 flex items-center gap-2 mb-3"><Camera size={18} /> Live Video</h3>
        ...
        <section className={`panel signals-panel flex flex-col transition-all duration-300 ${!isSignalsCollapsed ? '!flex-1 w-full overflow-y-auto custom-scrollbar' : 'h-min overflow-hidden'}`}>
          <div className="flex justify-between items-center mb-1">
             <h3 className="text-lg font-semibold text-slate-100 flex items-center gap-2 m-0"><TrendingUp size={18} /> Emotion Trend Signals</h3>
             <button onClick={() => setIsSignalsCollapsed(!isSignalsCollapsed)} className="p-1.5 hover:bg-white/10 rounded-md text-slate-300 transition-colors" title={isSignalsCollapsed ? "Expand Signals Tracker" : "Collapse Signals Tracker"}>
                {isSignalsCollapsed ? <ChevronUp size={18}/>  : <ChevronDown size={18}/>}
             </button>
          </div>
          ...
          {!isSignalsCollapsed && (
            <>
              {signalHistory.length > 0 ? (
                <>
                  <div className="signal-legend mt-2">
              ...
              <div className="signal-chart mt-4">
              ...
              <div className="signal-averages mt-4">
                {SIGNAL_CONFIG.map((signal) => (
              ...
              <div className="signals-detail-grid mt-4">
                <div className="signals-detail-panel">
                  <h4 className="flex items-center gap-2 text-sm font-semibold text-slate-300 mb-2"><Activity size={16} /> Action Units</h4>
                ...
                <div className="signals-detail-panel">
                  <h4 className="flex items-center gap-2 text-sm font-semibold text-slate-300 mb-2"><Eye size={16} /> Head Pose & Gaze</h4>
                  {metrics ? (
                    <>
            ...
            <p style={{color: '#64748b', fontSize:'0.9rem'}} className="mt-4">Signal graph will appear once face metrics start streaming.</p>
          )}
          </>
          )}
        </section>
